# test_reduce_v1_e2e.py
# Run:
#   python test_reduce_v1_e2e.py
#
# (Optional) to avoid the TORCH_CUDA_ARCH_LIST warning and speed up compilation:
#   TORCH_CUDA_ARCH_LIST="9.0a" python test_reduce_v1_e2e.py
...
# ---------------------------------------------------------------------
# Make arch explicit if user didn't set it (helps compilation time).
# Keep this BEFORE load_inline().
# ---------------------------------------------------------------------
...
def reduce_v1_triton(
    ...
    for kb in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        a = tl.load(
            a_ptrs,
            mask=token_mask[:, None] & ((k_base + offs_k[None, :]) < K),
            other=0.0,
        )
        b = tl.load(
            b_ptrs,
            mask=((k_base + offs_k[:, None]) < K) & (offs_n[None, :] < N),
            other=0.0,
        )
        acc += tl.dot(a, b)
...
def triton_forward(A, B, sorted_ids, expert_ids, topk_w, top_k, mul_routed_weight=False, num_warps=8):
    ...
    # In this toy harness: sorted_ids length == EM (no padding)
...
# ---------------------------------------------------------------------
# CUDA WMMA warp-tiled extension (corrected)
# ---------------------------------------------------------------------
cuda_src = r'''
...
__global__ void reduce_v1_wmma_warp_tiled(
  ...
  for (int k0 = 0; k0 < K; k0 += BK) {
    ...
    if (active && (n0 + 15 < N)) {
      ...
      // B: interpret B^T as col-major with ld=K (B is row-major N×K)
      wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::col_major> b_frag;
      const half* b_tile_ptr = B + e * stride_be + n0 * stride_bn + k0 * stride_bk;
  ...
  // Store each warp tile to shared then scatter
  __shared__ float SmemAcc[16 * 16 * 16]; // 16 tiles * 256 floats = 4096 floats
...
torch::Tensor reduce_v1_cuda(
  ...
  // num_tokens_post_padded = sorted_ids.numel()
  ...
  if (mul_routed_weight) {
    reduce_v1_wmma_warp_tiled<true><<<gridDim, block>>>(
    ...
    reduce_v1_wmma_warp_tiled<false><<<gridDim, block>>>(
      A_ptr, B_ptr, C_ptr, W_ptr, sorted_ptr, expert_ptr, ntpp_ptr,
      N, K, EM, EM,
      stride_am, stride_ak,
      stride_be, stride_bn, stride_bk,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("reduce_v1_cuda", &reduce_v1_cuda, "reduce_v1 CUDA (WMMA warp-tiled)");
}
'''

ext = load_inline(
    name="reduce_v1_ext_wmma_warp_tiled",
    cpp_sources="",
    cuda_sources=cuda_src,
    functions=None,  # using PYBIND11_MODULE
    with_cuda=True,
...
def main():
    ...
    # Memory info (helps confirm B fits)
    ...
    # Toy routing: identity and per-M-block expert assignment (touches only num_pid_m experts)
    ...
    # Correctness: compare Triton vs CUDA outputs (same toy routing)
    C_tri = triton_forward(A, B, sorted_ids, expert_ids, topk_w, top_k, mul_routed_weight=False, num_warps=8)
    C_cu2d = cuda_forward(A, B, sorted_ids, expert_ids, topk_w, top_k, mul_routed_weight=False)
    ...
    # FLOPs + TFLOPS
    flops = 2.0 * EM * N * K
    tri_tflops = flops / (tri_ms * 1e-3) / 1e12
    cu_tflops = flops / (cu_ms * 1e-3) / 1e12
    print(f"TFLOPS: Triton {tri_tflops:.3f}, CUDA {cu_tflops:.3f}")

    # Effective bandwidth estimate (based on a simple bytes-touched model)
    # In this toy harness, only num_pid_m experts are used.
    bytes_B = (num_pid_m * N * K) * 2  # fp16
    bytes_A = (EM * K) * 2
    bytes_C = (EM * N) * 2
    ...
    print(f"Assumed bytes/call: {bytes_total/1e6:.3f} MB (A {bytes_A/1e6:.3f}, B {bytes_B/1e6:.3f}, C {bytes_C/1e6:.3f})")
    print(f"Effective BW: Triton {tri_bw:.1f} GB/s, CUDA {cu_bw:.1f} GB/s")
